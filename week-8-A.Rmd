...
```{r}
treatment_2 <- stan_glm(data = week_8,
                    att_chg ~ treatment - 1, 
                    family = gaussian,
                    refresh = 0)
# If you look at the trains data, you will see that that those under control are more liberal compared to those under treatment
```
...
treatment_2 %>%
  ...
  geom_density(aes(y = after_stat(count/sum(count))),color = "blue") +
  geom_density(aes(x = treatmentControl, y = after_stat(count/sum(count)), color = "red"), show.legend = FALSE) +
  labs(title = "Posterior Probablity Distribution",
       subtitle = "Average change in attitude towards immigration",
       y = "Probability",
       x = "Scale") +
  theme_classic() 
  

# There is greater shift in the probability of attitude change for those under treated condition
...
# Represents change in ATE in the treatment group minus the ATE in the control group. This posterior distribution represents probability of receiving varying ATEs 
```
